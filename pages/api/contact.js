            ...
            <html> 
              <head> 
                ...
                <style type="text/css"> 
                  ...
                  .form-container{
                    margin-bottom: 24px; padding: 20px; border: 1px dashed #ccc;
                  }
                  .form-heading{
                    color: #2a2a2a;
...
export default async function handler(req, res){
    if (req.method === "POST") {
      ...
      const status = await fetch(
        `https://www.google.com/recaptcha/api/siteverify?secret=${process.env.RECAPTCHA_SECRET}&response=${recaptcha_token}`,
        {method: "POST"}
      ).then(res => res.json())
      ...
      try {
        await transporter.sendMail({
          ...
          subject: `Automated message from ${process.env.SITE_URL}`,
        })
